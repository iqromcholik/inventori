@extends('layouts.app', ['title' => 'Edit Pengeluaran Barang'])
        ...
        <div class="row g-4">
            <div class="col-sm-12">
                <div class="bg-light rounded h-100 p-4">
                    <h6 class="mb-4">Edit Pengeluaran Barang</h6>
                    <form action="{{ route('pengeluaran.update', $pengeluaranBarang->id_pengeluaran) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control @error('tgl_keluar') is-invalid @enderror"
                                ...
                                value="{{ old('tgl_keluar', $pengeluaranBarang->tgl_keluar) }}">
                            @error('tgl_keluar')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control  @error('tujuan') is-invalid @enderror" name="tujuan"
                                id="tujuan" value="{{ old('tujuan', $pengeluaranBarang->tujuan) }}">
                            @error('tujuan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <select class="form-select mb-3 @error('kode_barang') is-invalid @enderror" name="kode_barang"
                                ...
                                @foreach ($kodeBarangs as $barang)
                                    <option value="{{ $barang->kode_barang }}"
                                        {{ $pengeluaranBarang->kode_barang == $barang->kode_barang ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control  @error('kuantity') is-invalid @enderror"
                                name="kuantity" id="kuantity" value="{{ old('kuantity', $pengeluaranBarang->kuantity) }}">
                            @error('kuantity')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
