@extends('layouts.app', ['title' => 'Edit Order Barang'])
        ...
        <div class="row g-4">
            <div class="col-sm-12">
                <div class="bg-light rounded h-100 p-4">
                    <h6 class="mb-4">Edit Order Barang</h6>
                    <form action="{{ route('order.barang.update', $orderBarang->no_po) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control  @error('no_po') is-invalid @enderror" name="no_po"
                                id="no_po" value="{{ old('no_po') ?? $orderBarang->no_po }}">
                            @error('no_po')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control @error('tanggal') is-invalid @enderror" name="tanggal"
                                id="tanggal" value="{{ old('tanggal') ?? $orderBarang->tanggal }}">
                            @error('tanggal')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <select class="form-select mb-3 @error('kode_supplier') is-invalid @enderror"
                                ...
                                @foreach ($kodeSuppliers as $supplier)
                                    <option value="{{ $supplier->kode_supplier }}"
                                        {{ $orderBarang->kode_supplier == $supplier->kode_supplier ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control  @error('ppn') is-invalid @enderror" name="ppn"
                                id="ppn" value="{{ old('ppn') ?? $orderBarang->ppn }}">
                            @error('ppn')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
